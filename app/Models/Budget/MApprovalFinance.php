...
class MApprovalFinance extends Model
{
	...
	public static function approve_finance($no_pengajuan, $user, $tgl)
	{
		$q = DB::table('bc_mst_pengajuan')
		->where('status_approval', '=', 'K')
		->where('no_pengajuan', '=', $no_pengajuan)
		->update([
			'status_approval' 		=> 'F',
			'user_approve_finance'	=> $user,
			'tgl_approve_finance'	=> $tgl
		]);

		return $q;
	}

	public static function save_barang($no_pengajuan, $rowid, $qty_finance, $user, $tgl)
	{
		...
		->where('b.status_approval', '=', 'K')
		->where('a.rowid', '=', $rowid)
		->update([
			'a.qty_finance'	=> $qty_finance,
			'a.qty_final'	=> $qty_finance,
			'a.user_update'	=> $user,
			'a.tgl_update'	=> $tgl
		]);
