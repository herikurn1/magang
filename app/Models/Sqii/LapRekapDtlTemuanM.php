...
class LapRekapDtlTemuanM extends Model
{
   ...
   public static function lap_detail_ageing_v2($kd_kawasan,$kd_cluster,$nik_petugas,$tot_unit,$periode_1,$periode_2,$tipe_ageing,$tahap_bangun)
    ...
    public static function search_dtl_temuan($kd_kawasan,$kd_cluster,$periode_1,$periode_2,$user_kontraktor)
    {
        ...
        switch ($user_kontraktor) {
          ...
          default:
            $kontraktor = "
                            INNER JOIN (SELECT KD_KAWASAN, KD_CLUSTER, BLOK, NOMOR , B.NAMA
                                        FROM SQII_ALOKASI_PENUGASAN A WITH (NOLOCK)
                                            INNER JOIN SQII_USER B WITH (NOLOCK)
                                                ON B.USER_ID = A.USER_ID
                                        WHERE 1=1 AND B.KD_JABATAN = 'KTT' AND KD_KAWASAN = '".$kd_kawasan."' AND KD_CLUSTER = '".$kd_cluster."' AND B.USER_ID LIKE '".$user_kontraktor."') B
                            ON B.KD_KAWASAN = A.KD_KAWASAN
                            AND B.KD_CLUSTER = A.KD_CLUSTER
                            AND B.BLOK = A.BLOK
                            AND B.NOMOR = A.NOMOR";
            $select_k = ", NM_KONTRAKTOR";
            $select_k2 = ", B.NAMA AS NM_KONTRAKTOR";
            $desc = "desc";
        }
        $sql = "
            SELECT 
                NM_KATEGORI_DEFECT,KD_ITEM_DEFECT,NM_ITEM_DEFECT, ROW_SPAN,
                SUM(S_JML_BI) S_JML_BI ,SUM(S_JML_QC) S_JML_QC , SUM(S_JML_SM) S_JML_SM,
                (SUM(S_JML_CLOSING) + SUM(S_JML_UNCLOSING)) S_JML, SUM(S_JML_CLOSING) S_JML_CLOSING, SUM(S_JML_UNCLOSING) S_JML_UNCLOSING,
                SUM(B_JML_BI) B_JML_BI ,SUM(B_JML_QC) B_JML_QC , SUM(B_JML_SM) B_JML_SM,
                (SUM(B_JML_CLOSING) + SUM(B_JML_UNCLOSING)) B_JML, SUM(B_JML_CLOSING) B_JML_CLOSING, SUM(B_JML_UNCLOSING) B_JML_UNCLOSING ".$select_k."
            FROM (
                SELECT NM_KATEGORI_DEFECT,B.KD_ITEM_DEFECT, NM_ITEM_DEFECT, ROW_SPAN = (SELECT COUNT(*) FROM SQII_ITEM_DEFECT WHERE KD_KATEGORI_DEFECT = A.KD_KATEGORI_DEFECT)
                    ,CASE WHEN D.KD_JABATAN = 'BI' THEN 1 ELSE 0 END AS S_JML_BI 
                    ,CASE WHEN D.KD_JABATAN = 'QC' THEN 1 ELSE 0 END AS S_JML_QC
                    ,CASE WHEN D.KD_JABATAN = 'SM' THEN 1 ELSE 0 END AS S_JML_SM
                    ,ISNULL(JML_CLOSING,0) AS S_JML_CLOSING
                    ,ISNULL(JML_UNCLOSING,0) AS S_JML_UNCLOSING
                    ,0 B_JML_BI 
                    ,0 B_JML_QC
                    ,0 B_JML_SM
                    ,0 B_JML_CLOSING
                    ,0 B_JML_UNCLOSING ".$select_k."
                FROM SQII_KATEGORI_DEFECT A WITH (NOLOCK)
                    INNER JOIN SQII_ITEM_DEFECT B WITH (NOLOCK)
                        ON B.KD_KATEGORI_DEFECT = A.KD_KATEGORI_DEFECT
                    LEFT JOIN (
                        SELECT 
                            USER_ID, A.KD_KAWASAN, A.KD_CLUSTER, STATUS_DEFECT
                            , CASE WHEN ISNULL(FLAG_CLOSING,'N') = 'Y' THEN 1 WHEN ISNULL(FLAG_CLOSING,'N') = 'O' THEN 0 ELSE 0 END AS JML_CLOSING
                            , CASE WHEN ISNULL(FLAG_CLOSING,'N') = 'N' THEN 1 WHEN ISNULL(FLAG_CLOSING,'N') = 'O' THEN 1 ELSE 0 END AS JML_UNCLOSING
                            , KD_ITEM_DEFECT ".$select_k2."
                        FROM SQII_GOSHOW_FOTO A WITH (NOLOCK)
                            ".$kontraktor."
                        WHERE 1=1
                        ) C
                        ON C.KD_ITEM_DEFECT = B.KD_ITEM_DEFECT
                        AND KD_KAWASAN = '".$kd_kawasan."'
                        AND KD_CLUSTER = '".$kd_cluster."'
                        AND STATUS_DEFECT = 'S'
                    LEFT JOIN SQII_USER D
                ...
                SELECT NM_KATEGORI_DEFECT,B.KD_ITEM_DEFECT, NM_ITEM_DEFECT, ROW_SPAN = (SELECT COUNT(*) FROM SQII_ITEM_DEFECT WHERE KD_KATEGORI_DEFECT = A.KD_KATEGORI_DEFECT)
                    ...
                    ,0 S_JML_CLOSING
                    ,0 S_JML_UNCLOSING
                    ,CASE WHEN D.KD_JABATAN = 'BI' THEN 1 ELSE 0 END AS B_JML_BI 
                    ,CASE WHEN D.KD_JABATAN = 'QC' THEN 1 ELSE 0 END AS B_JML_QC
                    ,CASE WHEN D.KD_JABATAN = 'SM' THEN 1 ELSE 0 END AS B_JML_SM
                    ,ISNULL(JML_CLOSING,0) AS B_JML_CLOSING
                    ,ISNULL(JML_UNCLOSING,0) AS B_JML_UNCLOSING ".$select_k."
                FROM SQII_KATEGORI_DEFECT A WITH (NOLOCK)
                    INNER JOIN SQII_ITEM_DEFECT B WITH (NOLOCK)
                        ON B.KD_KATEGORI_DEFECT = A.KD_KATEGORI_DEFECT
                    LEFT JOIN (
            ...
            GROUP BY NM_KATEGORI_DEFECT, KD_ITEM_DEFECT, NM_ITEM_DEFECT, ROW_SPAN ".$select_k."
            ORDER BY NM_KATEGORI_DEFECT, KD_ITEM_DEFECT, NM_ITEM_DEFECT, ROW_SPAN ".$select_k." ".$desc."
        ";  
        // DD($sql);
        $q = DB::connection('sqii2')
        ->select($sql);
