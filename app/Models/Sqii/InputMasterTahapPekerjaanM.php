...
class InputMasterTahapPekerjaanM extends Model
{
    ...
    public static function simpan_dt($kd_kawasan,$kd_cluster,$nm_tahap,$user){
        $q = DB::connection('sqii2')
        ->select(
            "
        ...
        ->table('SQII_MST_TAHAP_PEKERJAAN')
        ->insert(
        [
            'KD_TAHAP'      => $kd,
            'NM_TAHAP'      => $nm_tahap,
    ...
    public static function upd_fg($kd_tahap,$fg_aktif,$user){
        $result = DB::connection('sqii2')
        ->table('SQII_MST_TAHAP_PEKERJAAN')
        ->where('KD_TAHAP','=',$kd_tahap)
        ->update(
