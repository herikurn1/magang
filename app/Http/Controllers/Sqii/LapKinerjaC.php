...
class LapKinerjaC extends Controller
{
    ...
    public function index(Request $r)
    {
    	...
    	return view('sqii.LapKinerjaV')->with('dt', $dt);
    }
    ...
    public function show_data_item_defect(Request $r)
	{
		...
		$q = LapKinerjaM::show_data_item_defect($kd_kategori); 
		foreach ($q as $row) {
			$data[] = array(
	...
	public function save(Request $r)
	{
		...
		if($savebtnval == 'create'){
			$q = LapKinerjaM::simpan_dt($nm_item_defect,$kd_kategori_defect,$user);
		}else{
			$q = LapKinerjaM::update_dt($kd_item_defect,$nm_item_defect,$user);
		}
	...
	public function delete_dt(Request $r)
	{
		...
		$q = LapKinerjaM::delete_dt($kd_item_defect,$user);
	...
	public function search_dt(Request $r)
	{
		...
		$isKunjungan = 'false';
		$jml_unit_st = 0;
		$q = LapKinerjaM::search_dt($kd_kawasan,$kd_cluster,$nik_petugas,$periode_1,$periode_2,$tahap_bangun);
		$q_st = LapKinerjaM::search_dt_st($kd_kawasan,$kd_cluster,$nik_petugas,$tahap_bangun);

		if($q_st[0]->TOTAL_ST) $jml_unit_st = $q_st[0]->TOTAL_ST;

		foreach ($q as $row) {
			$q_cek_kunjungan = LapKinerjaM::data_kunjungan($kd_kawasan,$kd_cluster,$row->USER_ID_BAWAHAN,$periode_1,$periode_2,$tahap_bangun);
			foreach ($q_cek_kunjungan as $row_kunjungan) {
				if($row_kunjungan->JML > 0){$isKunjungan = 'true';}
			}
			if($isKunjungan == 'true'){
				$q_formula = LapKinerjaM::lap_cycle_time_v2($kd_kawasan,$kd_cluster,$row->USER_ID_BAWAHAN,$row->JML_UNIT,$periode_1,$periode_2,'2',$tahap_bangun);
				// dd($q_formula);die;
				$sum_c = 0;
				$sum_f = 0;
				$sum_e = 0;
				...
				foreach ($q_formula as $row_formula) {
					/*
					  tgl  : 23/05/2020
					  desc : perhitungan rumus CT lama di remark dulu
					  req  : permintaan team QC via pak sidiq 
					...
					if($row_formula->SISA_STOK > 0){
						...
						$d = 0;
						//$e = 0;
						$e = 0;//$row->JML_UNIT / $row_formula->JML_HR;
					}else{
						$c = $row_formula->JML_HR;
						$d = $sum_c / $row_formula->CYCLE_COUNT;
						//$e = $row->JML_UNIT / $d;
						$e = $row->JML_UNIT / $c; //$row->JML_UNIT / $row_formula->JML_HR;
					}
					// $d = $sum_c / $cycle_count;
					// $e = $row->JML_UNIT / $d;
					// $sum_f = ($sum_f + $e)/$n;
					...
					*/	
					...
					/*
					  tgl  : 23/05/2020
					  desc : perhitungan rumus CT di ambil dari nilai terakhir yg sudah closing
					  req  : permintaan team QC via pak sidiq 
					*/
					$sum_c = $sum_c + $row_formula->JML_HR;
					if($row_formula->SISA_STOK == 0){
				...
				$data[] = array(
					...
					'jml_defect'		=> $row->JML_DEFECT,
					'total_defect_f'	=> number_format($row->TOTAL_DEFECT, 2, '.', ''),
					'total_defect'		=> $row->TOTAL_DEFECT,
					'a1'				=> $row->A1,
					'a2'				=> $row->A2,
					'a3'				=> $row->A3,
					'avg_day'			=> number_format($sum_f, 2, '.', ''),
					// 'avg_day'			=> number_format($row->AVG_DAY, 2, '.', '')
					'jml_unit_st'		=> $jml_unit_st
				);
			}else{
				$sum_f = 0;
				$data[] = array(
	...
	public function nik_petugas(Request $r)
    {
    	...
    	$q = LapKinerjaM::nik_petugas($keyword,$kd_kawasan,$kd_cluster);
    ...
    public function lap_defect(Request $r)
	{
		...
		$periode_1   = $this->sysController->indokegabung($r->periode_1);
		$periode_2   = $this->sysController->indokegabung($r->periode_2);
		$q = LapKinerjaM::lap_defect($kd_kawasan,$kd_cluster,$user_id_bawahan,$tot_unit,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'nm_kategori_defect'	=> $row->NM_KATEGORI_DEFECT, 
				'kd_kategori_defect'	=> $row->KD_KATEGORI_DEFECT,
	...
	public function lap_detail_kualitas(Request $r)
	{
		...
		$periode_1   		= $this->sysController->indokegabung($r->periode_1);
		$periode_2   		= $this->sysController->indokegabung($r->periode_2);
		$q = LapKinerjaM::lap_detail_kualitas($kd_kawasan,$kd_cluster,$user_id_bawahan,$kd_kategori_defect,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'blok'					=> $row->BLOK,
				'nomor'					=> $row->NOMOR,
	...
	public function lap_formulir_kualitas_bangunan(Request $r)
	{
		...
		$periode_1   		= $r->periode_1;
		$periode_2   		= $r->periode_2;
		$q = LapKinerjaM::lap_formulir_kualitas_bangunan($kd_kawasan,$kd_cluster,$user_id_bawahan,$kd_kategori_defect,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'blok'					=> $row->BLOK,
				'nomor'					=> $row->NOMOR,
	...
	public function data_tahap(Request $r)
	{
		$kd_kawasan   = $r->kd_kawasan;
		$kd_cluster   = $r->kd_cluster;
		$q = LapKinerjaM::data_tahap($kd_kawasan,$kd_cluster);
		foreach ($q as $row) {
			$data[] = array(
