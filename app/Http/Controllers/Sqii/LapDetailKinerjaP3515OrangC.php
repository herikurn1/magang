...
class LapDetailKinerjaP3515OrangC extends Controller
{
	...
	{ 
		...
		$q = LapKinerjaP3515M::lap_defect($kd_kawasan,$kd_cluster,$user_id_bawahan,$jml_unit,$periode_1,$periode_2,$tahap_bangun);
		...
		foreach ($q as $row) {
            ...
            $row_tbl .= '
            <tr onclick="detail_kinerja_p3515_tahap(\''.$kd_kawasan.'\',\''.$kd_cluster.'\',\''.$nm_kawasan.'\',\''.$nm_cluster.'\',\''.$nm_sm.'\',\''.$periode_1l.'\',\''.$periode_2l.'\',\''.$user_id.'\', \''.$user_id_bawahan.'\', \''.$nama.'\', \''.$jml_unit.'\', \''.$jml_defect.'\', \''.$total_defect.'\', \''.$tot_unit.'\', \''.$row->KD_TAHAP.'\', \''.$tahap_bangun.'\')">
            	<td>'.$no++.'</td>
            	<td><div class="link_cursor">'.$row->NM_TAHAP.'</div></td>
            	<td style="text-align: center;">'.number_format($jml_unit, 0, '.', '').'</td>
            	<td style="text-align: center;">'.number_format($row->JML_TIDAK_SESUAI, 0, '.', '').'</td>
            	<td style="text-align: center;">'.number_format($row->NILAI_TIDAK_SESUAI, 2, '.', '').'</td>
            </tr>';
		}
		...
		if(isset($row_tbl)){// LapDetailKualitasV LapDefectV2
			return view('sqii.LapDetailKinerjaP3515OrangV')
				->with('dt', $dt)
				->with('tbl', $tbl);
		}
	}
	...
	public function print_dt(Request $r)
   {
		...
		$q = LapKinerjaP3515M::lap_defect($kd_kawasan,$kd_cluster,$user_id_bawahan,$jml_unit,$per_exp_1,$per_exp_2,$tahap_bangun);
