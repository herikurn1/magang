...
class LapRekapProgresC extends Controller
{
    ...
    public function index(Request $r)
    {
    	...
    	$q = LapRekapProgresM::data_kawasan(); 
    	...
    	return view('sqii.LapRekapProgresV')->with('dt', $dt);
    }
    ...
    public function show_data_item_defect(Request $r)
	{
		...
		$q = LapRekapProgresM::show_data_item_defect($kd_kategori); 
		foreach ($q as $row) {
			$data[] = array(
	...
	public function save(Request $r)
	{
		...
		if($savebtnval == 'create'){
			$q = LapRekapProgresM::simpan_dt($nm_item_defect,$kd_kategori_defect,$user);
		}else{
			$q = LapRekapProgresM::update_dt($kd_item_defect,$nm_item_defect,$user);
		}
	...
	public function delete_dt(Request $r)
	{
		...
		$q = LapRekapProgresM::delete_dt($kd_item_defect,$user);
	...
	public function search_dt(Request $r)
	{
		...
		$isKunjungan 	= 'false';
		$jml_unit_st 	= 0;
		$q = LapRekapProgresM::search_dt($kd_kawasan,$kd_cluster,$kd_periode,$tahap_bangun);
		...
		foreach ($q as $row) {
			...
			$data[] = array(
				...
				'bobot_rencana'		=> number_format($row->BOBOT_RENCANA, 2, '.', ''),
				'bobot_aktual'		=> number_format($row->BOBOT_AKTUAL, 2, '.', ''),
				'calc_aktual'		=> number_format($row->BOBOT_AKTUAL * $row->JML_UNIT, 2, '.', ''),
	...
	public function print_dt(Request $r)
    {
		...
		$tahap_bangun   = $r->tahap_bangun;
		$q = LapRekapProgresM::search_dt($kd_kawasan,$kd_cluster,$nik_petugas,$periode_1,$periode_2,$tahap_bangun);
		$no = 1;
		$tot_unit = 0;	
		$tot_defect = 0;
		...
		foreach ($q as $row) {
			...
			$q_formula = LapRekapProgresM::lap_cycle_time_v2($kd_kawasan,$kd_cluster,$row->USER_ID_BAWAHAN,$row->JML_UNIT,$periode_1,$periode_2,'2',$tahap_bangun);
			$sum_c = 0;
			$sum_f = 0;
			$sum_e = 0;
			$n 	   = 1;
	...
	public function nik_petugas(Request $r)
    {
    	...
    	$q = LapRekapProgresM::nik_petugas($keyword,$kd_kawasan,$kd_cluster);
    ...
    public function lap_defect(Request $r)
	{
		...
		$periode_1   = $this->sysController->indokegabung($r->periode_1);
		$periode_2   = $this->sysController->indokegabung($r->periode_2);
		$q = LapRekapProgresM::lap_defect($kd_kawasan,$kd_cluster,$user_id_bawahan,$tot_unit,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'nm_kategori_defect'	=> $row->NM_KATEGORI_DEFECT, 
				'kd_kategori_defect'	=> $row->KD_KATEGORI_DEFECT,
	...
	public function lap_detail_kualitas(Request $r)
	{
		...
		$periode_1   		= $this->sysController->indokegabung($r->periode_1);
		$periode_2   		= $this->sysController->indokegabung($r->periode_2);
		$q = LapRekapProgresM::lap_detail_kualitas($kd_kawasan,$kd_cluster,$user_id_bawahan,$kd_kategori_defect,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'blok'					=> $row->BLOK,
				'nomor'					=> $row->NOMOR,
	...
	public function lap_formulir_kualitas_bangunan(Request $r)
	{
		...
		$periode_1   		= $r->periode_1;
		$periode_2   		= $r->periode_2;
		$q = LapRekapProgresM::lap_formulir_kualitas_bangunan($kd_kawasan,$kd_cluster,$user_id_bawahan,$kd_kategori_defect,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'blok'					=> $row->BLOK,
				'nomor'					=> $row->NOMOR,
	...
	public function data_tahap(Request $r)
	{
		$kd_kawasan   = $r->kd_kawasan;
		$kd_cluster   = $r->kd_cluster;
		$q = LapRekapProgresM::data_tahap($kd_kawasan,$kd_cluster);
		foreach ($q as $row) {
			$data[] = array(
	...
	public function data_periode(Request $r)
	{
		$kd_kawasan   = $r->kd_kawasan;
		$kd_cluster   = $r->kd_cluster;
		$tahap_bangun   = $r->tahap_bangun;
		$q = LapRekapProgresM::data_periode($kd_kawasan,$kd_cluster,$tahap_bangun);
		foreach ($q as $row) {
			$data[] = array(
