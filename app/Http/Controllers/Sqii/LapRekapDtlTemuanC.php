...
use App\Models\Sqii\LapRekapDtlTemuanM;

ini_set('max_execution_time', 0); 
set_time_limit(0);

class LapRekapDtlTemuanC extends Controller
{
    ...
    public function index(Request $r)
    {
    	...
    	$q = LapRekapDtlTemuanM::data_kawasan(); 
    	...
    	return view('sqii.LapRekapDtlTemuanV')->with('dt', $dt);
    }
    ...
    public function show_data_item_defect(Request $r)
	{
		...
		$q = LapRekapDtlTemuanM::show_data_item_defect($kd_kategori); 
		foreach ($q as $row) {
			$data[] = array(
	...
	public function save(Request $r)
	{
		...
		if($savebtnval == 'create'){
			$q = LapRekapDtlTemuanM::simpan_dt($nm_item_defect,$kd_kategori_defect,$user);
		}else{
			$q = LapRekapDtlTemuanM::update_dt($kd_item_defect,$nm_item_defect,$user);
		}
	...
	public function delete_dt(Request $r)
	{
		...
		$q = LapRekapDtlTemuanM::delete_dt($kd_item_defect,$user);
	...
	public function search_dt(Request $r)
	{
		...
		$isKunjungan = 'false';
		$jml_unit_st = 0;
		$q = LapRekapDtlTemuanM::search_dtl_temuan($kd_kawasan,$kd_cluster,$periode_1,$periode_2,$user_kontraktor);
		...
		foreach ($q as $row) {
			$data[] = array(
				...
				's_jml_bi'			=> $row->S_JML_BI,
				's_jml_qc'			=> $row->S_JML_QC,
				's_jml_sm'			=> $row->S_JML_SM,
				's_jml'				=> $row->S_JML,
				's_jml_closing'		=> $row->S_JML_CLOSING,
				's_jml_unclosing'	=> $row->S_JML_UNCLOSING,
				'b_jml_bi'			=> $row->B_JML_BI,
				'b_jml_qc'			=> $row->B_JML_QC,
				'b_jml_sm'			=> $row->B_JML_SM,
				'b_jml'				=> $row->B_JML,
				'b_jml_closing'		=> $row->B_JML_CLOSING,
				'b_jml_unclosing'	=> $row->B_JML_UNCLOSING
			);
		}
	...
	public function print_dt(Request $r)
    {
		...
		$q = LapRekapDtlTemuanM::search_dt($kd_kawasan,$kd_cluster,$nik_petugas,$periode_1,$periode_2,$tahap_bangun);
		$no = 1;
		$tot_unit = 0;	
		$tot_defect = 0;
		...
		foreach ($q as $row) {
			...
			$q_formula = LapRekapDtlTemuanM::lap_cycle_time_v2($kd_kawasan,$kd_cluster,$row->USER_ID_BAWAHAN,$row->JML_UNIT,$periode_1,$periode_2,'2',$tahap_bangun);
			$sum_c = 0;
			$sum_f = 0;
			$sum_e = 0;
			$n 	   = 1;
    ...
    public function lap_defect(Request $r)
	{
		...
		$periode_1   = $this->sysController->indokegabung($r->periode_1);
		$periode_2   = $this->sysController->indokegabung($r->periode_2);
		$q = LapRekapDtlTemuanM::lap_defect($kd_kawasan,$kd_cluster,$user_id_bawahan,$tot_unit,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'nm_kategori_defect'	=> $row->NM_KATEGORI_DEFECT, 
				'kd_kategori_defect'	=> $row->KD_KATEGORI_DEFECT,
	...
	public function lap_detail_kualitas(Request $r)
	{
		...
		$periode_1   		= $this->sysController->indokegabung($r->periode_1);
		$periode_2   		= $this->sysController->indokegabung($r->periode_2);
		$q = LapRekapDtlTemuanM::lap_detail_kualitas($kd_kawasan,$kd_cluster,$user_id_bawahan,$kd_kategori_defect,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'blok'					=> $row->BLOK,
				'nomor'					=> $row->NOMOR,
	...
	public function lap_formulir_kualitas_bangunan(Request $r)
	{
		...
		$periode_1   		= $r->periode_1;
		$periode_2   		= $r->periode_2;
		$q = LapRekapDtlTemuanM::lap_formulir_kualitas_bangunan($kd_kawasan,$kd_cluster,$user_id_bawahan,$kd_kategori_defect,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'blok'					=> $row->BLOK,
				'nomor'					=> $row->NOMOR,
	...
	public function data_kontraktor(Request $r)
	{
		$kd_kawasan   = $r->kd_kawasan;
		$kd_cluster   = $r->kd_cluster;
		$q = LapRekapDtlTemuanM::data_kontraktor($kd_kawasan,$kd_cluster);
		foreach ($q as $row) {
			$data[] = array(
	...
	public function nik_petugas(Request $r)
    {
    	...
    	$q = LapRekapDtlTemuanM::nik_petugas($keyword,$kd_kawasan,$kd_cluster);
