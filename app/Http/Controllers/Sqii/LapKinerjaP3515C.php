...
use App\Models\Sqii\LapKinerjaP3515M;

ini_set('max_execution_time', 0); 
set_time_limit(0);

class LapKinerjaP3515C extends Controller
{
    ...
    public function index(Request $r)
    {
    	...
    	$q = LapKinerjaP3515M::data_kawasan(); 
    	...
    	return view('sqii.LapKinerjaP3515V')->with('dt', $dt);
    }
    ...
    public function show_data_item_defect(Request $r)
	{
		...
		$q = LapKinerjaP3515M::show_data_item_defect($kd_kategori); 
		foreach ($q as $row) {
			$data[] = array(
	...
	public function save(Request $r)
	{
		...
		if($savebtnval == 'create'){
			$q = LapKinerjaP3515M::simpan_dt($nm_item_defect,$kd_kategori_defect,$user);
		}else{
			$q = LapKinerjaP3515M::update_dt($kd_item_defect,$nm_item_defect,$user);
		}
	...
	public function delete_dt(Request $r)
	{
		...
		$q = LapKinerjaP3515M::delete_dt($kd_item_defect,$user);
	...
	public function search_dt(Request $r)
	{
		...
		$isKunjungan = 'false';
		$jml_unit_st = 0;
		$q = LapKinerjaP3515M::search_dt($kd_kawasan,$kd_cluster,$nik_petugas,$periode_1,$periode_2,$tahap_bangun);
	...
	public function print_dt(Request $r)
    {
		...
		foreach ($q as $row) {
			...
			$q_formula = LapKinerjaP3515M::lap_cycle_time_v2($kd_kawasan,$kd_cluster,$row->USER_ID_BAWAHAN,$row->JML_UNIT,$periode_1,$periode_2,'2',$tahap_bangun);
			$sum_c = 0;
			$sum_f = 0;
			$sum_e = 0;
			$n 	   = 1;
	...
	public function nik_petugas(Request $r)
    {
    	...
    	$q = LapKinerjaP3515M::nik_petugas($keyword,$kd_kawasan,$kd_cluster);
    ...
    public function lap_defect(Request $r)
	{
		...
		$periode_1   = $this->sysController->indokegabung($r->periode_1);
		$periode_2   = $this->sysController->indokegabung($r->periode_2);
		$q = LapKinerjaP3515M::lap_defect($kd_kawasan,$kd_cluster,$user_id_bawahan,$tot_unit,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'nm_kategori_defect'	=> $row->NM_KATEGORI_DEFECT, 
				'kd_kategori_defect'	=> $row->KD_KATEGORI_DEFECT,
	...
	public function lap_detail_kualitas(Request $r)
	{
		...
		$periode_1   		= $this->sysController->indokegabung($r->periode_1);
		$periode_2   		= $this->sysController->indokegabung($r->periode_2);
		$q = LapKinerjaP3515M::lap_detail_kualitas($kd_kawasan,$kd_cluster,$user_id_bawahan,$kd_kategori_defect,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'blok'					=> $row->BLOK,
				'nomor'					=> $row->NOMOR,
	...
	public function lap_formulir_kualitas_bangunan(Request $r)
	{
		...
		$periode_1   		= $r->periode_1;
		$periode_2   		= $r->periode_2;
		$q = LapKinerjaP3515M::lap_formulir_kualitas_bangunan($kd_kawasan,$kd_cluster,$user_id_bawahan,$kd_kategori_defect,$periode_1,$periode_2);
		foreach ($q as $row) {
			$data[] = array(
				'blok'					=> $row->BLOK,
				'nomor'					=> $row->NOMOR,
	...
	public function data_tahap(Request $r)
	{
		$kd_kawasan   = $r->kd_kawasan;
		$kd_cluster   = $r->kd_cluster;
		$q = LapKinerjaP3515M::data_tahap($kd_kawasan,$kd_cluster);
		foreach ($q as $row) {
			$data[] = array(
