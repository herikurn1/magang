...
use App\Models\Budget\MRevBudget;
use Illuminate\Support\Facades\Route;
class CRevBudget extends Controller
{
	...
	public function index(Request $r)
	{
		...
		return view('budget.vRevBudget')->with('dt', $dt);
	}
	...
	public function save(Request $r)
	{
		...
		if($act == "add"){
			$q = MRevBudget::insert_dt($thn_anggaran, $kd_departemen, $kd_unit, $kd_lokasi, $user, $tgl);
			$no_pengajuan = $q;
			echo $no_pengajuan;
		}else{
			$q = MRevBudget::update_dt($no_pengajuan, $user, $tgl);
		}
...
// var_dump($add_kd_barang);
		if($add_nm_barang != null){
			$add_total 	= count($add_nm_barang);

			for ($i=0; $i < $add_total; $i++) { 
				$y = MRevBudget::count_no_pengajuan($no_pengajuan, $thn_anggaran, $kd_departemen, $kd_unit, $kd_lokasi, $user);
				...
				if(trim($no_pengajuan) != ""){
					if($add_nm_barang[$i] == ""){
						...
						$insert_barang = MRevBudget::insert_barang($no_pengajuan, $no_budget, $add_kd_barang[$i], $add_nm_barang[$i], $add_kd_kategori_budget[$i], $add_nm_kategori_budget[$i], $add_kd_kategori[$i], $add_nm_kategori[$i], $add_kd_jenis[$i], $add_nm_jenis[$i], $add_qty[$i], $add_harga[$i], $add_jumlah_harga[$i], $add_urgency[$i], $add_catatan[$i], $user, $tgl);
					}
					if($err > 0){
		...
		if($edt_rowid != null){
			...
			$status_approval = '';
			$get_mst_pengajuan = MRevBudget::get_mst_pengajuan($no_pengajuan);
				...
				for ($i=0; $i < $edt_total; $i++) {
					if(trim($no_pengajuan) != ""){
						...
						if(($edt_qty[$i] != $qty) || ($edt_harga[$i] != $harga) || ($edt_jumlah_harga[$i] != $jumlah_harga) || ($edt_urgency[$i] != $urgency) || ($edt_catatan[$i] != $catatan)){
							$edit_barang = MRevBudget::edit_barang($edt_rowid[$i], $edt_qty[$i], $edt_harga[$i], $edt_jumlah_harga[$i], $edt_urgency[$i], $edt_catatan[$i], $user, $tgl);
						}
					}
				}
			}
	...
	public function search_dt(Request $r)
	{
		...
		$q = MRevBudget::search_dt($user, $kd_unit, $kd_lokasi, $keyword);
		foreach ($q as $row) {
	...
	public function search_barang(Request $r)
	{
		...
		$q = MRevBudget::search_barang($keyword, $kd_kategori, $kd_jenis);
	...
	public function delete_dt(Request $r)
	{
		...
		$q = MRevBudget::delete_dt($no_pengajuan, $user, $tgl);
	...
	public function delete_barang(Request $r)
	{
		...
		if($status_approval == "E"){
			$q = MRevBudget::delete_barang($rowid);
	...
	public function submit_kabag(Request $r)
	{
		...
		$q = MRevBudget::submit_kabag($no_pengajuan, $user, $tgl);
	}
	...
	public function view_history(Request $r)
	{
		...
		$q = MRevBudget::view_history($thn_anggaran, $kd_unit, $kd_departemen, $kd_barang);
