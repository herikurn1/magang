...
use App\Models\Budget\MApprovalBod;

class CApprovalBod extends Controller
{
	...
	public function index(Request $r)
	{
		...
		return view('budget.vApprovalBod')->with('dt', $dt);
	}
	...
	public function show_pengajuan($r, $status_approval)
	{
		...
		$q = MApprovalBod::show_pengajuan($thn_anggaran, $kd_unit, $status_approval);
		foreach ($q as $row) {
	...
	public function save(Request $r)
	{
		...
		if($no_pengajuan != null){
			...
			for ($i=0; $i < $total; $i++) { 
				$q = MApprovalBod::approve_bod($no_pengajuan[$i], $user, $tgl);
			}
		}
	}
		...
		if($rowid != null){
			...
			for ($i=0; $i < $total; $i++) { 
    			...
    			foreach ($get_dtl_before as $get_dtl_before_row) {
    				$qty_final_before = $get_dtl_before_row->qty_final;

    				if($qty_final[$i] != $qty_final_before){
    					$q = MApprovalBod::save_barang($no_pengajuan, $rowid[$i], $qty_final[$i], $user, $tgl);
    				}
    			}
			}
		}
