...
@section('js')
	<script type="text/javascript">
			...
			//if(act != "add"){
				$('#tbl_item tbody').prepend(''+
					'<tr>'+
						...
						'</td>'+
						'<td>'+
							// '<div id="label_barang_'+no+'" class="input-group">'+
							// 	'<input type="text" class="form-control form-control-sm" placeholder="Pilih Barang" readonly />'+
							// 	'<button type="button" class="btn btn-info btn-sm" onclick="search_barang(\''+no+'\')">...</button>'+
							// '</div>'+
							// '<div id="input_barang_'+no+'" style="display: none;" class="input-group">'+
							...
							// 	'<button type="button" class="btn btn-info btn-sm" onclick="search_barang(\''+no+'\')" disabled>...</button>'+
							// 	'<input type="hidden" name="add_kd_barang[]" id="add_kd_barang_'+no+'"  />'+
							// '</div>'+
						'</td>'+
						'<td>'+
				...
				no++;
			//}else{
			//	alert('Harap pilih salah satu pengajuan terlebih dahulu');
			//}
		}
						...
						var row = ''+
							'<tr>'+
                                ...
                                '<input type="text" name="copy_urgency[]" id="copy_urgency_'+y.rowid+'" class="form-control form-control-sm" readonly style="width: 150px; text-align: right;" value="'+y.nm_urgency+'" />'+
									// '<select name="copy_urgency[]" class="form-control form-control-sm" id="copy_urgency_'+y.rowid+'" style="width: 120px;" disabled>'+
							        // '</select>'+
								'</td>'+
								'<td>'+
		...
		function search_kategori_budget(id) {
			...
			$.ajax({
				type 	: 'POST',
				url 	: 'rev_budget/search_kategori_budget',
				data 	: {
					"_token"	: '{{ csrf_token() }}',
					"keyword"	: keyword
				},
		...
		function search_barang(id, id_kategori_budget, nm_kategori_budget, id_jenis, id_kategori) {
			...
			$.ajax({
				type 	: 'POST',
				url 	: 'rev_budget/search_barang?page='+search_barang_page,
				data 	: {
					"_token"		: '{{ csrf_token() }}',
					"keyword"		: keyword,
					"helper"		: helper,
		          	...
		          	'<div class="col-12">'+
		            	'<div class="card">'+
		              		'<div class="card-header">'+
								...
								'<i class="fas fa-copy"></i>copy data '+thn+''+ 
					            '</button></h3>'+
		              		'</div>'+
