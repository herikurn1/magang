...
@section('js')
	<script type="text/javascript">
		...
		function save_dt() {
			var data_all = $('#frm_approval_finance').serializeArray();

			$('#v_loading').show();
			$.ajax({
				type 	: 'POST',
				url 	: 'approval_finance/save',
				data 	: data_all,
		...
		function show_finance() {
			...
			$.ajax({
				type 	: 'POST',
				url 	: 'approval_finance/show_finance',
				data 	: {
		...
		function show_complete() {
			...
			$.ajax({
				type 	: 'POST',
				url 	: 'approval_finance/show_complete',
				data 	: {
					"thn_anggaran" 	: thn_anggaran,
					"kd_unit"		: kd_unit,
					"_token"		: '{{ csrf_token() }}'
		...
		function show_barang(id, status_approval) {
			...
			$.ajax({
				...
				success : function(msg) {
					...
					var i = 1;
					$.each(msg, function(x, y){
						var qty_finance_default 	= y.qty;
						var qty_finance_readonly 	= '';

						if(y.qty_finance != null){
							qty_finance_default = y.qty_finance;
						}

						if(y.qty_finance == "0"){
							qty_finance_readonly = 'readonly';
						}

						if(status_approval == "K"){
							btn_del_barang = '<button class="btn btn-warning btn-flat btn-sm" type="button" onclick="hapus_qty_finance(\''+y.rowid+'\')"><i class="fas fa-times"></i></button>';
						}
													...
													'<tr>'+
														...
														'<td class="align-middle">'+
															'<div class="input-group input-group-sm">'+
																...
																'<input type="text" class="form-control form-control-sm rounded-0" id="qty_finance_'+y.rowid+'" name="qty_finance[]" style="width: 80px; text-align: center;" value="'+qty_finance_default+'" min="1" '+qty_finance_readonly+' '+btn_edit_barang+' />'+
		...
		function hapus_qty_finance(id) {
			var qty_finance_status = $('#qty_finance_'+id).attr('readonly');

			if(!qty_finance_status){
				var ans = confirm('Apakah anda yakin ingin menolak item ini ?');
				if(ans){
					$('#qty_finance_'+id).val('0');
					$('#qty_finance_'+id).attr('readonly', true);
				}
			}
		}
		...
		function save_barang() {
			...
			$.ajax({
				type 	: 'POST',
				url 	: 'approval_finance/save_barang',
				data 	: data_all,
				success : function(msg) {
	...
	<div class="row">
		<div class="col-sm-12">
			<div class="card card-primary card-outline">
		      	...
		      	<form class="form-horizontal" id="frm_approval_finance" autocomplete="off" onsubmit="return save_process()">
			        <div class="card-body">
					          	...
					          	<div class="card card-primary card-outline card-outline-tabs">
				              		<div class="card-header p-0 border-bottom-0">
										<ul class="nav nav-tabs" id="budget-tab" role="tablist">
											<li class="nav-item">
												<a class="nav-link active" id="budget-tab-finance" data-toggle="pill" href="#budget-tabContent-finance" role="tab" aria-controls="budget-tabContent-finance" aria-selected="true">Analisa Finance</i></a>
											</li>
											<li class="nav-item">
					              	...
					              	<div class="card-body table-responsive p-0" >
						                <div class="tab-content" id="budget-tabContent">
						                  	<div class="tab-pane fade show active" id="budget-tabContent-finance" role="tabpanel" aria-labelledby="budget-tab-finance" style="padding: 10px;">
