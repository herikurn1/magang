...
@section('js')
	...
	<script type="text/javascript">
		...
		function check_role_budget(func) {
			$.ajax({
				type 	: 'POST',
				url 	: 'rek_capex_cat_dept/check_role_budget',
				data 	: {
					"func"		: func,
					"_token"	: '{{ csrf_token() }}'
				},
		...
		function search_unit_budget() {
			...
			$.ajax({
				type 	: 'POST',
				url 	: 'rek_capex_cat_dept/search_unit_budget',
				data 	: {
					"_token"	: '{{ csrf_token() }}',
					"keyword"	: keyword
				},
		...
		function search_departemen_budget() {
			...
			$.ajax({
				type 	: 'POST',
				url 	: 'rek_capex_cat_dept/search_departemen_budget',
				data 	: {
					"_token"	: '{{ csrf_token() }}',
					"keyword"	: keyword
				},
		...
		function search_detail() {
			...
			$.ajax({
				type	: 'POST',
				url 	: 'rek_capex_cat_dept/search_detail',
				data 	: data_all,
				success	: function (msg) {
					$('#v_loading').hide();
				...
				$.ajax({
						type: 'GET',
						url: 'rek_capex_cat_dept/search_chart',
						data: data_all,
						cache: false,
						success: function(msg){
				...
				'<div class="row">'+
		          	'<div class="col-12">'+
	              		'<div class="card-body table-responsive p-0" style="height: 400px;">'+
	              			'<center>'+
							  	'<h4>PT Summarecon Agung TBK</h4>'+
		              			'<h4>Rekap CAPEX per Dept <span id="span_thn_anggaran"></span></h4>'+
		              		'</center>'+
	              			'<hr>'+
