...
@section('js')
    ...
    function show_tahap_pekerjaan() {
      ...
      $.ajax({
        type  : 'POST',
        url   : 'input_tbl_tahap_pekerjaan_c/show_tahap_pekerjaan',
        data  : {
          "kd_unit"   : kd_unit,
          "kd_lokasi" : kd_lokasi,
        ...
        success : function(msg) {          
          ...
          $.each(msg, function(x, y) {
            if(y.fg_aktif == 'A'){ 
              chek_var = '';
              chek_var2 = '<button type="button" class="btn btn-warning  btn-sm" onclick="upd_fg(\''+kd_kawasan+'\',\''+kd_cluster+'\',\''+y.kd_tahap+'\',\''+y.fg_aktif+'\')">Disable</button> ';
              style = '';
            }else{ 
              chek_var = '<button type="button" class="btn btn-success btn-sm" onclick="upd_fg(\''+kd_kawasan+'\',\''+kd_cluster+'\',\''+y.kd_tahap+'\',\''+y.fg_aktif+'\')">Enable</button> ';
              chek_var2 = '';
    ...
    function delete_dt(kd_tahap) {
        $.ajax({
          ...
          url: "input_tbl_tahap_pekerjaan_c/delete_dt",
          type: "POST",
    ...
    function upd_fg(kd_kawasan,kd_cluster,kd_tahap,fg_aktif) {
        ...
        $.ajax({
          ...
          url: "input_tbl_tahap_pekerjaan_c/upd_fg",
          type: "POST",
    ...
    function save_process() {
      ...
      var data_all      = $('#CustomerForm').serializeArray();
        ...
        $.ajax({
          data: data_all,
          url: "input_tbl_tahap_pekerjaan_c/save",
          type: "POST",
          ...
          error: function (data) {
              alert('proses simpan di batalkan')
              console.log('Error:', data);
    ...
    function data_cluster() {
        ...
        $.ajax({
            type: 'POST',
            url: 'input_tbl_tahap_pekerjaan_c/data_cluster',
            cache: false,
            data: {
              "_token"  : '{{ csrf_token() }}',
              "kd_unit" : kd_unit,
  ...
  function mst_tahap_pekerjaan() {
    var kd_kawasan = $('#kd_kawasan').val();
    var kd_cluster = $('#kd_cluster').val()    
      
      $.ajax({
          type: 'POST',
          url: 'input_tbl_tahap_pekerjaan_c/mst_tahap_pekerjaan',
          cache: false,
          data: {
            "_token"  : '{{ csrf_token() }}',
            "kd_unit" : kd_unit,
          ...
          success: function(msg){
              ...
              row = '<select class="form-control col-sm-12" name="id_tahap_pekerjaan" id="id_tahap_pekerjaan">';
              $.each(msg, function(x, y) {
                row += '<option value="'+ y.kd_tahap +'#'+ y.nm_tahap +'">'+y.kd_tahap+' # '+ y.nm_tahap +'</option>';
              });
              row += '</select>';

              $('#div_mst_tahap_pekerjaan').html(row);
		...
		<div class="col-sm-12">
			<div class="card card-primary card-outline">
              ...
              <nav class="navbar justify-content-between">
                <a class="form-brand">
                  <h3 class="card-title">Input Tabel Tahap Pekerjaan</h3>
                </a>
                ...
                <form id="CustomerForm" name="CustomerForm" class="form-horizontal" autocomplete="off" onsubmit="return save_process()">
                   ...
                   <input type="hidden" name="kd_cluster_l" id="kd_cluster_l" value="">
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                          <div class="" id="div_mst_tahap_pekerjaan"></div>
