...
@section('js')
    ...
    function show_jenis_bangunan() {
      ...
      $.ajax({
        type  : 'POST',
        url   : 'register_data_tp_bangunan_c/show_jenis_bangunan',
        data  : {
          "kd_unit"   : kd_unit,
          "kd_lokasi" : kd_lokasi,
          "_token"    : '{{ csrf_token() }}'
    ...
    function sync_dt() {
      ...
      $.ajax({
        type  : 'POST',
        url   : 'register_data_tp_bangunan_c/sync_dt',
        data  : {
          "_token"  : '{{ csrf_token() }}',
          "kd_unit" : kd_unit,
          "kd_lokasi" : kd_lokasi,
    ...
    function data_cluster() {
        ...
        $.ajax({
            type: 'POST',
            url: 'register_data_tp_bangunan_c/data_cluster',
            cache: false,
            data: {
              "_token"  : '{{ csrf_token() }}',
              "kd_unit" : kd_unit,
    ...
    function search_dt() {
      ...
      $.ajax({
        type  : 'POST',
        url   : 'register_data_tp_bangunan_c/search_dt',
        data  : {
          "_token"  : '{{ csrf_token() }}',
          "kd_unit" : kd_unit,
          "kd_lokasi" : kd_lokasi,
        ...
        success : function(msg) {
          ...
          $("#tbl_kawasan").find("tr:gt(0)").remove(); // CLEAR TABLE <div class="link_cursor">'+y.kd_tipe+'</div>
          ...
          $.each(msg, function(x, y) {
            row += ''+
              '<tr>'+
                '<td>'+ no++ +'</td>'+
                '<td>'+y.nm_jenis+'</td>'+
                '<td onclick="data_denah(\''+kd_kawasan+'\',\''+y.kd_jenis+'\',\''+y.kd_tipe+'\',\''+y.nm_jenis+'\',\''+y.nm_tipe+'\',\''+nm_kawasan+'\',\''+nm_cluster+'\')"><div class="link_cursor">'+y.kd_tipe+'</div></td>'+
                '<td>'+y.nm_tipe+'</td>'+
                '<td>'+y.jml_lantai+'</td>'+
              '</tr>'+
            '';
    ...
    function data_denah(kd_kawasan,kd_jenis,kd_tipe,nm_jenis,nm_tipe,nm_kawasan,nm_cluster) {
      kd_kawasan_l  = kd_kawasan;
      kd_jenis_l    = kd_jenis;
      kd_tipe_l     = kd_tipe;  
      nm_jenis_l    = nm_jenis;  
      nm_tipe_l     = nm_tipe;  
      nm_kawasan_l  = nm_kawasan;  
      nm_cluster_l  = nm_cluster;  

      $('#v_loading').show();

      $.ajax({
        type  : 'POST',
        url   : 'register_data_tp_bangunan_c/data_denah',
        data  : {
          "_token"  : '{{ csrf_token() }}',
          "kd_unit" : kd_unit,
          "kd_lokasi" : kd_lokasi,
        ...
        success : function(msg) {
          ...
          $('#div_denah').show();
          $("#tbl_denah").find("tr:gt(0)").remove(); // CLEAR TABLE

          $.each(msg, function(x, y) {
            //$("#formulir_denah").attr('src','https://sqii.gadingemerald.com/public/'+y.path_foto_denah+''+y.src_foto_denah);
            //console.log(y.nm_lantai);
            row += ''+
              '<tr>'+
                '<td>'+ no++ +'</td>'+
                '<td>'+y.nm_lantai+'</td>'+
    ...
    function available_stok() {
      ...
      var row;
        ...
        $.ajax({
            type: 'POST',
            url: 'register_data_tp_bangunan_c/available_stok',
            cache: false,
            data: {
              "_token"  : '{{ csrf_token() }}',
              "kd_unit" : kd_unit,
    ...
    function file_upload(dataall) {
      ...
      $.ajax({
        data: formData,
        url: "register_data_tp_bangunan_c/file_upload",
        type: "POST",
        cache:false,
        dataType: false,
        processData: false,
        ...
        success: function (data) {
            ...
            data_denah(kd_kawasan_l,kd_jenis_l,kd_tipe_l,nm_jenis_l,nm_tipe_l,nm_kawasan_l,nm_cluster_l);
        },
        error: function (data) {
    ...
    function delete_dt2(kd_lantai,kd_kawasan,kd_jenis,kd_tipe) {
        $.ajax({
          ...
          url: "register_data_tp_bangunan_c/delete_denah",
          type: "POST",

          success: function (data) {
            data_denah(kd_kawasan,kd_jenis,kd_tipe);
          },
          error: function (data) {
              console.log('Error:', data);
          }
	...
	<div class="row">
		<div class="col-sm-12">
          ...
          <nav class="navbar justify-content-between">
            <a class="form-brand">
              <h3 class="card-title">Register Tipe Bangunan, Denah Lantai & Kuota Foto</h3>
            </a>
        ...
        <div class="card-body">
          ...
          <div class="card-body table-responsive p-0" style="display: none; height: 300px;" id="div_kawasan">
            <table id="tbl_kawasan" class="table table-head-fixed table-bordered table-hover text-nowrap">
              <thead>
                <tr>
                  <th style="width: 10px">#</th>
                  <th style="text-align: center;">Jenis Bgn</th>
                  <th style="text-align: center;">Kd Tipe</th>
          ...
          <div class="form-group">
            <label for="kd_kawasan" class="col-sm-12"></label>
          </div>        
          <div id="div_denah" class="card-body" style="display: none;">  
          <ul class="nav nav-tabs" id="custom-content-below-tab" role="tablist">
           <!--  <li class="nav-item">
              <a class="nav-link active" id="content_model_tab" data-toggle="pill" href="#content_model_tab" role="tab" aria-controls="custom-concontent_model_tab" aria-selected="true">Model</a>
            </li> -->
            <li class="nav-item">
              <a class="nav-link active" id="custom-content-below-home-tab" data-toggle="pill" href="#custom-content-below-home" role="tab" aria-controls="custom-content-below-home" aria-selected="true">Denah</a>
            </li>
          </ul>
          <div class="tab-content" id="custom-content-below-tabContent">
            ...
            <table id="tbl_denah" class="table table-bordered table-hover" >
              <thead>                  
                <tr>
                  <th style="width: 10px">#</th>
