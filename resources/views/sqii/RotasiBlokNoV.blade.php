...
@section('js')
    ...
    function data_blok_no() {
      ...
      $.ajax({
        type  : 'POST',
        url   : 'rotasi_blok_no_c/data_blok_no',
        data  : {
          "kd_unit"   : kd_unit,
          "kd_lokasi" : kd_lokasi,
            ...
            row += ''+
              '<tr>'+
                ...
                '<td style="text-align: center;">'+
                  '<button type="button" class="btn btn-info btn-sm" onclick="add_staff(\''+y.blok+'\', \''+y.nomor+'\')">Ganti Petugas</button></td>'+
              '</tr>'+
            '';
          });
    ...
    function data_cluster() {
        ...
        $.ajax({
            type: 'POST',
            url: 'rotasi_blok_no_c/data_cluster',
            cache: false,
            data: {
              "_token"  : '{{ csrf_token() }}',
              "kd_unit" : kd_unit,
    ...
    function available_stok() {
      ...
      $.ajax({
        type  : 'POST',
        url   : 'rotasi_blok_no_c/available_stok',
        data  : {
        "kd_kawasan" : kd_kawasan,
        "kd_cluster" : kd_cluster, 
        "nik_petugas" : nik_petugas,
    ...
    function delete_dt(kd_kawasan,kd_cluster,blok,nomor,user_id) {
        $.ajax({
          ...
          url: "rotasi_blok_no_c/delete_dt",
          type: "POST",
  ...
  function nik_petugas() {
    ...
    $.ajax({
      type  : 'POST',
      url   : 'rotasi_blok_no_c/nik_petugas?page='+sys_search_unit_page,
      data  : {
        "_token"  : '{{ csrf_token() }}',
        "keyword" : keyword,
        "kd_kawasan" : kd_kawasan
  ...
  function add_staff(blok,nomor) {
    get_staff(blok,nomor);
    // $('#CustomerForm').trigger("reset");
    nm_petugas = $('#nm_petugas').val();
    $('#modelHeadingRotasi').html("Ganti Petugas");
  ...
  function get_staff(blok,nomor) {
      var row;
      user_lama   = $('#nik_petugas').val();
      kd_jabatan  = $('#kd_jabatan_petugas').val();
      
      $.ajax({
          type: 'POST',
          url: 'rotasi_blok_no_c/get_staff',
          cache: false,
          data: {
            "_token"  : '{{ csrf_token() }}',
            "kd_unit" : kd_unit,
  ...
  function save_process2() {
    var kd_kawasan  = $('#kd_kawasan').val();
    var kd_cluster  = $('#kd_cluster').val();
    var user_lama   = $('#nik_petugas').val();
    var user_baru   = $('#id_bawahan').val().split('#');

    $.ajax({
        data  : {
          "user_baru"     : user_baru[0],
          "user_lama"     : user_lama,
          "kd_kawasan"    : kd_kawasan,
          "kd_cluster"    : kd_cluster,
          "blok"          : user_baru[3],
      ...
      url: "rotasi_blok_no_c/save_rotasi",
      type: "POST",
  ...
  <div class="card card-primary card-outline">
          <div class="card-header">
            <nav class="navbar justify-content-between">
              <a class="form-brand">
                <h3 class="card-title">Rotasi Blok/Nomor</h3>
              </a>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title nav-item d-none d-sm-inline-block " id="modelHeadingRotasiBlokNo"></h4>
            </div>
            <div class="modal-body">
                <form id="BawahanForm" name="BawahanForm" class="form-horizontal" autocomplete="off" onsubmit="return save_process2()">
                   ...
                   <input type="hidden" name="kd_lantai" id="kd_lantai" value="">
                    <div class="form-group">
                        <label for="stok_add" class="col-sm-6 control-label">Petugas Baru</label>
                        <div class="col-sm-12">
                          <div class="row">
                            <div class="col-6">
