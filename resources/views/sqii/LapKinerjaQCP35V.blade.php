...
@section('js')
  <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    ...
    function item_defect() {
      ...
      $.ajax({
        type  : 'POST',
        url   : 'lap_kinerja_qc_P35_c/show_data_item_defect',
        data  : {
    ...
    function delete_dt(kd_item_defect) {
        $.ajax({
          ...
          url: "lap_kinerja_qc_P35_c/delete_dt",
          type: "POST",
    ...
    function data_cluster() {
        ...
        $.ajax({
            type: 'POST',
            url: 'lap_kinerja_qc_P35_c/data_cluster',
            cache: false,
            data: {
              "_token"  : '{{ csrf_token() }}',
              "kd_unit" : kd_unit,
    ...
    function data_tahap() {
        ...
        $.ajax({
            type: 'POST',
            url: 'lap_kinerja_qc_P35_c/data_tahap',
            cache: false,
            data: {
              "_token"  : '{{ csrf_token() }}',
              "kd_unit" : kd_unit,
    ...
    function search_dt() {
      ...
      $.ajax({
        type  : 'POST',
        url   : 'lap_kinerja_qc_P35_c/search_dt',
        data  : {
          "kd_unit"   : kd_unit,
          "kd_lokasi" : kd_lokasi,
          "kd_kawasan" : kd_kawasan,
        ...
        success : function(msg) {          
          ...
          $.each(msg, function(x, y) {
            row += ''+
              '<tr style="">'+
                '<td>'+ no++ +'</td>'+
                '<td onclick="detail_kinerja_qc_p35_orang(\''+kd_kawasan+'\',\''+kd_cluster+'\',\''+nm_kawasan+'\',\''+nm_cluster+'\',\''+nm_sm+'\',\''+periode_1+'\',\''+periode_2+'\',\''+y.user_id+'\', \''+y.user_id_bawahan+'\', \''+y.nama+'\', \''+y.jml_unit+'\', \''+y.jml_tdk_sesuai+'\', \''+y.nilai_tdk_sesuai+'\', \''+tot_unit+'\', \''+tahap_bangun+'\')"><div class="link_cursor">'+y.nama+'</div></td>'+
  ...
  function nik_petugas() {
    ...
    $.ajax({
      type  : 'POST',
      url   : 'lap_kinerja_qc_P35_c/nik_petugas?page='+sys_search_kry_page,
      data  : {
        "_token"  : '{{ csrf_token() }}',
        "keyword" : keyword,
        "kd_kawasan" : kd_kawasan,
  ...
  function print_dt(){
    ...
    $.ajax({
      ...
      url: '{{ url("sqii/lap_kinerja_qc_P35_c/") }}/print_dt',
      type: "POST",
      dataType: 'html',
  ...
  function detail_kinerja_qc_p35_orang(kd_kawasan, kd_cluster, nm_kawasan, nm_cluster, nm_sm, periode_1, periode_2, user_id,user_id_bawahan, nama, jml_unit, jml_defect, total_defect, tot_unit, tahap_bangun){
    ...
    window.open('{{ url("sqii/lap_detail_kinerja_qc_P35_orang_c/") }}'+'/'+kd_kawasan+'/'+kd_cluster+'/'+nm_kawasan+'/'+nm_cluster+'/'+nm_sm+'/'+periode_1+'/'+periode_2+'/'+user_id+'/'+user_id_bawahan+'/'+nama+'/'+jml_unit+'/'+jml_defect+'/'+total_defect+'/'+tot_unit+'/'+session_user_id+'/'+tahap_bangun); 
  }
	</script>
@endsection
		...
		<div class="col-sm-12">
			<div class="card card-primary card-outline">
              ...
              <nav class="navbar justify-content-between">
                <a class="form-brand">
                  <h3 class="card-title">Laporan Kinerja QC P35</h3>
                </a>
